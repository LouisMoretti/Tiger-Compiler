...
% require "3.8"
  ...
  % define api.prefix{parse} % define api.namespace {parse}
...
#define yylex                                                                  \
  lexer                                                                        \
    .lex // Within bison's parse() we should invoke lexer.lex(), not the global lex()
}
...
record_attr : ID[name] "=" exp[value] "," record_attr[attributes]
{
  $$ = $attributes;
  $$->insert($$->begin(), make_FieldInit(@$, $name, $value));
}
| ID[name] "=" exp[value]
{
...
func_prms : exp[value] "," func_prms[prms]
{
  $$ = $prms;
  $$->insert($$->begin(), $value);
}
| exp[value] { $$ = make_exps_type($value); }
// End Fix

% token EXP "_exp";
exp : INT { $$ = make_IntExp(@$, $1); }
...
| lvalue { $$ = $1; }
| ID[name] "(" func_prms[prms] ")" { $$ = make_CallExp(@$, $name, $prms); }
| ID[name] "("
           ")"
{
...
| "(" exps[expressions] ")" { $$ = make_SeqExp(@$, $expressions); }
...
| EXP "(" INT[id] ")" { $$ = metavar<ast::Exp>(td, $id); };
// End Fix

// Start Fix
% token LVALUE "_lvalue";
lvalue : ID[name] { $$ = make_SimpleVar(@$, $name); }
| lvalue.big | LVALUE "(" INT[id] ")" { $$ = metavar<ast::Var>(td, $id); };

lvalue.big : ID[var] "." ID[attr]
{
...
| lvalue.big[var] "." ID[attr] { $$ = make_FieldVar(@$, $var, $attr); }
...
{
  $$ = $list;
  $$->splice_front(*metavar<ast::ChunkList>(td, $id));
};
// End Fix

// Start Fix
funcdec
  : "function" ID[name] "(" tyfields[fields] ")"
                                             ":" typeid[type] "=" exp[body]
{
  $$ = make_FunctionChunk(@$);
  ...
  for (auto field : *$fields)
    {
      varchunk->emplace_back(
        *make_VarDec(@$, field->name_get(), &field->type_name_get(), nullptr));
    }
...
vardec : "var" ID[name] ":" typeid[type] ":=" exp[value]
{
  $$ = make_VarChunk(@$);
  $$->emplace_back(*make_VarDec(@$, $name, $type, $value));
}
| "var" ID[name] ":=" exp[value]
{
  $$ = make_VarChunk(@$);
  $$->emplace_back(*make_VarDec(@$, $name, nullptr, $value));
}
| "var" ID[name] ":" typeid[type]
{
  $$ = make_VarChunk(@$);
  $$->emplace_back(
    *make_VarDec(@$, $name, $type, make_StringExp(@$, "nullptr")));
}
| "var" ID[name]
{
  $$ = make_VarChunk(@$);
  $$->emplace_back(
    *make_VarDec(@$, $name, nullptr, make_StringExp(@$, "nullptr")));
};
// End Fix
...
tydec : "type" ID "=" ty { $$ = make_TypeDec(@$, $2, $4); };
...
tyfields : % empty { $$ = make_fields_type(); }
| tyfields.1 { $$ = $1; };

tyfields.1 : tyfields.1 "," tyfield
{
  $$ = $1;
  $$->emplace_back($3);
}
...
tyfield : ID ":" typeid { $$ = make_Field(@$, $1, $3); };

% token NAMETY "_namety";
typeid : ID { $$ = make_NameTy(@$, $1); }
  ...
  void parse::parser::error(const location_type& l, const std::string& m)
{
  ...
  td.error_ << "Parsing Error was encountered at line " << l << ": " << m
            << std::endl;
}
